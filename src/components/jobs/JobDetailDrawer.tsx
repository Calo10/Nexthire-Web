...
import JobAdDesignModal from './JobAdDesignModal';
...
export default function JobDetailDrawer({ isOpen, job, onClose, onUpdated, onDeleted }: JobDetailDrawerProps) {
  ...
  const [adDesignMode, setAdDesignMode] = useState<'generate' | 'view' | null>(null);
  ...
  const jobHasAdDesign = Boolean(job?.hasAdDesign);
  ...
  return (
          ...
          <div className="pt-4 border-t border-gray-200">
            ...
            <div className="flex flex-wrap gap-2">
              ...
              <Button
                ...
                size="sm"
                disabled={!job || isSaving}
                onClick={() => setAdDesignMode(jobHasAdDesign ? 'view' : 'generate')}
              >
                <span className="inline-flex items-center gap-2">
                  ...
                  {jobHasAdDesign ? t('jobs.drawer.viewAdDesign') : t('jobs.drawer.generateAdDesign')}
                </span>
              </Button>
            </div>
          </div>
    ...
    />
    {adDesignMode === 'generate' && job ? (
      <JobAdDesignModal
        mode="generate"
        isOpen
        jobId={String(job.id)}
        jobTitle={job.title || ''}
        onClose={() => setAdDesignMode(null)}
        onSaved={() => {
          onUpdated?.({ ...job, hasAdDesign: true });
          setAdDesignMode(null);
        }}
      />
    ) : null}
    {adDesignMode === 'view' && job ? (
      <JobAdDesignModal
        mode="view"
        isOpen
        jobId={String(job.id)}
        ...
        onClose={() => setAdDesignMode(null)}
        onDeleted={() => {
          onUpdated?.({ ...job, hasAdDesign: false });
          setAdDesignMode(null);
        }}
      />
    ) : null}
